"""
Gradio playground UI for a trained Transformer LM.

This module holds the importable core — model loading, streaming
generation closures, and the Blocks UI — so that both the local CLI
(``scripts/playground.py``) and the HuggingFace Spaces entrypoint
(``spaces/app.py``) can use it from the installed ``cs336_basics``
package without the project having to ship a top-level ``scripts``
package in its wheel.

Requires the ``ui`` optional dependency (``pip install cs336-basics[ui]``);
gradio is imported only when this module is.
...
Up to four tabs:
  ...
  3. Aligned          (shown when SFT / DPO checkpoints are passed)
                      same prompt fed to base, SFT, and DPO side by
                      side — visualises what post-training did.
